...
input embedding as E𝐸E, the final hidden vector of the special [CLS] token as C∈ℝH𝐶superscriptℝ𝐻C\in\mathbb{R}^{H}, and the final hidden vector for the ithsuperscript𝑖thi^{\rm th} input token as Ti∈ℝHsubscript𝑇𝑖superscriptℝ𝐻T_{i}\in\mathbb{R}^{H}.
...
(labeled as NotNext).
As we show in Figure 1, C𝐶C is used for next sentence prediction (NSP).555The final model achieves 97%-98% accuracy on NSP. Despite its simplicity, we demonstrate in Section 5.1 that pre-training towards this task is very beneficial to both QA and NLI.
...
To fine-tune on GLUE, we represent the input sequence (for single sentence or sentence pairs) as described in Section 3, and use the final hidden vector C∈ℝH𝐶superscriptℝ𝐻C\in\mathbb{R}^{H} corresponding to the first input token ([CLS]) as the aggregate representation.
...
The probability of word i𝑖i being the start of the answer span is computed as a dot product between Tisubscript𝑇𝑖T_{i} and S𝑆S followed by a softmax over all of the words in the paragraph: Pi=eS⋅Ti∑jeS⋅Tjsubscript𝑃𝑖superscript𝑒⋅𝑆subscript𝑇𝑖subscript𝑗superscript𝑒⋅𝑆subscript𝑇𝑗P_{i}=\frac{e^{S{\cdot}T_{i}}}{\sum_{j}e^{S{\cdot}T_{j}}}. The analogous formula is used for the end of the answer span. The score of a candidate span from position i𝑖i to position j𝑗j is defined as S⋅Ti+E⋅Tj⋅𝑆subscript𝑇𝑖⋅𝐸subscript𝑇𝑗S{\cdot}T_{i}+E{\cdot}T_{j}, and the maximum scoring span where j≥i𝑗𝑖j\geq i is used as a prediction. The training objective is the sum of the log-likelihoods of the correct start and end positions. We fine-tune for 3 epochs with a learning rate of 5e-5 and a batch size of 32.
...
BERTBASEBASE{}_{\textsc{BASE}} (Single)
80.8
88.5
-
-

BERTLARGELARGE{}_{\textsc{LARGE}} (Single)
84.1
90.9
-
-

BERTLARGELARGE{}_{\textsc{LARGE}} (Ensemble)
85.8
91.8
-
-

BERTLARGELARGE{}_{\textsc{LARGE}} (Sgl.+TriviaQA)
84.2
91.1
85.1
91.8

BERTLARGELARGE{}_{\textsc{LARGE}} (Ens.+TriviaQA)
86.2
92.2
87.4
93.2
...
We use a simple approach to extend the SQuAD v1.1 BERT model for this task. We treat questions that do not have an answer as having an answer span with start and end at the [CLS] token. The probability space for the start and end answer span positions is extended to include the position of the [CLS] token. For prediction, we compare the score of the no-answer span: s𝚗𝚞𝚕𝚕=S⋅C+E⋅Csubscript𝑠𝚗𝚞𝚕𝚕⋅𝑆𝐶⋅𝐸𝐶s_{\tt null}=S{\cdot}C+E{\cdot}C to the score of the best non-null span si,j^^subscript𝑠𝑖𝑗\hat{s_{i,j}} = 𝚖𝚊𝚡j≥i​S⋅Ti+E⋅Tj⋅subscript𝚖𝚊𝚡𝑗𝑖𝑆subscript𝑇𝑖⋅𝐸subscript𝑇𝑗{\tt max}_{j\geq i}S{\cdot}T_{i}+E{\cdot}T_{j}. We predict a non-null answer when si,j^>s𝚗𝚞𝚕𝚕+τ^subscript𝑠𝑖𝑗subscript𝑠𝚗𝚞𝚕𝚕𝜏\hat{s_{i,j}}>s_{\tt null}+\tau, where the threshold τ𝜏\tau is selected on the dev set to maximize F1. We did not use TriviaQA data for this model. We fine-tuned for 2 epochs with a learning rate of 5e-5 and a batch size of 48.
...
We fine-tune the model for 3 epochs with a learning rate of 2e-5 and a batch size of 16. Results are presented in Table 4. BERTLARGELARGE{}_{\textsc{LARGE}} outperforms the authors’ baseline ESIM+ELMo system by +27.1% and OpenAI GPT by 8.3%.
...
Results are presented in Table 7. BERTLARGELARGE{}_{\textsc{LARGE}} performs competitively with state-of-the-art methods. The best performing method concatenates the token representations from the top four hidden layers of the pre-trained Transformer, which is only 0.3 F1 behind fine-tuning the entire model. This demonstrates that BERT is effective for both fine-tuning and feature-based approaches.
...
Input
=[CLS] the man went to [MASK] store [SEP]absent[CLS] the man went to [MASK] store [SEP]\displaystyle=\text{\tt{[CLS] the man went to [MASK] store [SEP]}}

he bought a gallon [MASK] milk [SEP]

Label
=IsNextabsentIsNext\displaystyle=\text{\tt{IsNext}}

Input
=[CLS] the man [MASK] to the store [SEP]absent[CLS] the man [MASK] to the store [SEP]\displaystyle=\text{\tt{[CLS] the man [MASK] to the store [SEP]}}

penguin [MASK] are flight ##less birds [SEP]

Label
=NotNextabsentNotNext\displaystyle=\text{\tt{NotNext}}
...
We train with batch size of 256 sequences (256 sequences * 512 tokens = 128,000 tokens/batch) for 1,000,000 steps, which is approximately 40 epochs over the 3.3 billion word corpus. We use Adam with learning rate of 1e-4, β1=0.9subscript𝛽10.9{\beta}_{1}=0.9, β2=0.999subscript𝛽20.999{\beta}_{2}=0.999, L2 weight decay of 0.010.010.01, learning rate warmup over the first 10,000 steps, and linear decay of the learning rate. We use a dropout probability of 0.1 on all layers. We use a gelu activation Hendrycks and Gimpel (2016) rather than the standard relu, following OpenAI GPT. The training loss is the sum of the mean masked LM likelihood and the mean next sentence prediction likelihood.

Training of BERTBASEBASE{}_{\textsc{BASE}} was performed on 4 Cloud TPUs in Pod configuration (16 TPU chips total).141414https://cloudplatform.googleblog.com/2018/06/Cloud-TPU-now-offers-preemptible-pricing-and-global-availability.html Training of BERTLARGELARGE{}_{\textsc{LARGE}} was performed on 16 Cloud TPUs (64 TPU chips total). Each pre-training took 4 days to complete.
...
Figure 4. Our task-specific models are formed by incorporating BERT with one additional output layer, so a minimal number of parameters need to be learned from scratch. Among the tasks, (a) and (b) are sequence-level tasks while (c) and (d) are token-level tasks. In the figure, E𝐸E represents the input embedding, Tisubscript𝑇𝑖T_{i} represents the contextual representation of token i𝑖i, [CLS] is the special symbol for classification output, and [SEP] is the special symbol to separate non-consecutive token sequences.
...
Recognizing Textual Entailment is a binary entailment task similar to MNLI, but with much less training data Bentivogli et al. (2009).151515Note that we only report single-task fine-tuning results in this paper. A multitask fine-tuning approach could potentially push
...
Winograd NLI is a small natural language inference dataset Levesque et al. (2011). The GLUE webpage notes that there are issues with the construction of this dataset, 161616https://gluebenchmark.com/faq and every trained system that’s been submitted to GLUE has performed worse than the 65.1 baseline accuracy of predicting the majority class. We therefore exclude this set to be fair to OpenAI GPT. For our GLUE submission, we always predicted the majority class.

## Appendix C Additional Ablation Studies

### C.1 Effect of Number of Training Steps

Figure 5 presents MNLI Dev accuracy after fine-tuning from a checkpoint that has been pre-trained for k𝑘k steps. This allows us to answer the following questions:
...
Answer: Yes, BERTBASEBASE{}_{\textsc{BASE}} achieves almost 1.0% additional accuracy on MNLI when trained on 1M steps compared to 500k steps.
...
Generated on Fri Mar 8 07:11:12 2024 by LaTeXML
