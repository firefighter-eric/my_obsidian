...
Figure 1 gives a graphical overview of the top-down transformer, instantiating the proposed framework. We introduce its details in this section. Suppose a document has N𝑁N tokens, 𝒕={ti}i=1N𝒕superscriptsubscriptsubscript𝑡𝑖𝑖1𝑁\bm{t}=\{t_{i}\}_{i=1}^{N}. In our method, token representations are inferred by combining top-down and bottom-up inference. This leads to effective and efficient inference for token representations. They are then attended by a decoder to generate a summary, as in a regular encoder-decoder transformer.

### 2.1 Bottom-Up Inference

In the bottom-up inference, contextual embeddings of the tokens, {ei∣ei∈ℝd}i=1Nsuperscriptsubscriptconditional-setsubscript𝑒𝑖subscript𝑒𝑖superscriptℝ𝑑𝑖1𝑁\{e_{i}\mid e_{i}\in\mathbb{R}^{d}\}_{i=1}^{N}, are computed with N1subscript𝑁1N_{1} layers of local self-attention. In particular, each token tisubscript𝑡𝑖t_{i} only attends to nearby tokens within a window of size of w𝑤w. The complexity is hence O​(N​w)𝑂𝑁𝑤O(Nw), in contrast to O​(N2)𝑂superscript𝑁2O(N^{2}) for a full self-attention models. Please see Supplementary B.1 for more details.

### 2.2 Top-Down Inference

The efficiency with local self-attention in the bottom-up inference nevertheless comes with a limitation, that is, each eisubscript𝑒𝑖e_{i} only captures the context within a local window instead of that of the whole document. To mitigate this issue, we propose a top-down inference for token representations.

Consider a two-level multi-scale latent structure for a document. The low level consists of token representations, {ei}i=1Nsuperscriptsubscriptsubscript𝑒𝑖𝑖1𝑁\{e_{i}\}_{i=1}^{N}, computed by the bottom-up inference. The top level consists of units at a coarser level. It is affordable to apply full self-attention at the top level due to its coarser granularity, allowing these top-level units to capture global document context. In our work, the self-attention mechanism for the top-level representations is simply the original multi-head self-attention proposed in Vaswani et al. (2017). Readers are referred to Vaswani et al. (2017) for details.

Denote the top level representations after self-attention update as {sj∣sj∈ℝd}j=1Msuperscriptsubscriptconditional-setsubscript𝑠𝑗subscript𝑠𝑗superscriptℝ𝑑𝑗1𝑀\{s_{j}\mid s_{j}\in\mathbb{R}^{d}\}_{j=1}^{M} (see Section 2.3 for details on top-level representation initialization methods). We can then update the bottom-up-inferred token representations with the top-level representations. This is achieved with N3subscript𝑁3N_{3} top-down inference layers, as illustrated by the middle panel in Figure 1. Each layer contains three transformations on {ei}subscript𝑒𝑖\{e_{i}\}: (1) token self-attention, (2) token-segment cross-attention, (3) feed-forward. (1) and (3) are the same as those in the bottom-up inference layers or regular self-attention layer with local attention. (2) implementing the cross-attention between the top and bottom levels is the critical operation. In particular, each eisubscript𝑒𝑖e_{i} is updated with cross-attention,

e~i=ei+LayerNorm​(∑j=1Mαi​j​fv​(sj)),αi​j=exp⁡(fq​(ei)T​fk​(sj))d​∑l=1Mexp⁡(fq​(ei)T​fk​(sl))formulae-sequencesubscript~𝑒𝑖subscript𝑒𝑖LayerNormsuperscriptsubscript𝑗1𝑀subscript𝛼𝑖𝑗subscript𝑓𝑣subscript𝑠𝑗subscript𝛼𝑖𝑗subscript𝑓𝑞superscriptsubscript𝑒𝑖𝑇subscript𝑓𝑘subscript𝑠𝑗𝑑superscriptsubscript𝑙1𝑀subscript𝑓𝑞superscriptsubscript𝑒𝑖𝑇subscript𝑓𝑘subscript𝑠𝑙\tilde{e}_{i}=e_{i}+{\rm LayerNorm}(\sum_{j=1}^{M}\alpha_{ij}f_{v}(s_{j})),\quad\alpha_{ij}=\frac{\exp{(f_{q}(e_{i})^{T}f_{k}(s_{j})})}{\sqrt{d}\sum_{l=1}^{M}\exp{(f_{q}(e_{i})^{T}f_{k}(s_{l}))}}

(1)

where fq,fk,subscript𝑓𝑞subscript𝑓𝑘f_{q},f_{k}, and fvsubscript𝑓𝑣f_{v} indicate query, key, and value linear mappings, respectively. For notional clarity, Equation 1 only illustrates the case with a single attention head. In practice, we use multi-heads. The cross-attention operation injects global contextual information into bottom-up-inferred token representations, eisubscript𝑒𝑖e_{i}, and yields global-context-aware token representations, e~isubscript~𝑒𝑖\tilde{e}_{i}, conditioning on which a summary can be generated by a decoder.
...
In the top-down inference, segment-level self-attention has a complexity of O​(M2)𝑂superscript𝑀2O(M^{2}), and token-segment cross-attention has a complexity of O​(N​M)𝑂𝑁𝑀O(NM). Thus, together with bottom-up inference, the complexity is O​(N​w+M2+N​M)𝑂𝑁𝑤superscript𝑀2𝑁𝑀O(Nw+M^{2}+NM). In practice, we use relatively small w𝑤w (window size) and M𝑀M (number of segments).

### 2.3 Pooling Methods

As aforementioned, we use a single top level, consisting of fixed-length segments, in the current work. The segment representations are initialized by pooling token representations. Following the notation above, suppose a document is divided into M𝑀M segments, and the embedding of the j𝑗jth segment is initialized as,

sj(0)=∑n=1kpn​ej×d+nsuperscriptsubscript𝑠𝑗0superscriptsubscript𝑛1𝑘subscript𝑝𝑛subscript𝑒𝑗𝑑𝑛s_{j}^{(0)}=\sum_{n=1}^{k}p_{n}e_{j\times d+n}

(2)

where k𝑘k is the kernel size and d𝑑d is the stride. pnsubscript𝑝𝑛p_{n} is the weight for the n𝑛nth token. We introduce two approaches to compute the weights. The first method is average pooling (AvgPool) and hence pn=1ksubscript𝑝𝑛1𝑘p_{n}=\frac{1}{k}, which is simple and convenient. In the second approach, we leverage the reference summary to define the importance of each token to assign adaptive weights (AdaPool). Particularly, we learn an importance tagger with labels constructed with the reference summaries, which involves three steps:
...
In our experiments, we also used OracleAdaPool where the weights are obtained from Step 1 with the reference summaries.
Note that if {pn}n=1ksuperscriptsubscriptsubscript𝑝𝑛𝑛1𝑘\{p_{n}\}_{n=1}^{k} does not form a valid probability distribution, sjsubscript𝑠𝑗s_{j} can be computed with a normalized weight distribution within each pooling window as follows,

sj(0)=∑n=1kexp⁡(pn)​ej×d+n∑n=1kexp⁡(pn).superscriptsubscript𝑠𝑗0superscriptsubscript𝑛1𝑘subscript𝑝𝑛subscript𝑒𝑗𝑑𝑛superscriptsubscript𝑛1𝑘subscript𝑝𝑛s_{j}^{(0)}=\frac{\sum_{n=1}^{k}\exp(p_{n})e_{j\times d+n}}{\sum_{n=1}^{k}\exp(p_{n})}.

(3)

{sj(0)}j=1Msuperscriptsubscriptsuperscriptsubscript𝑠𝑗0𝑗1𝑀\{s_{j}^{(0)}\}_{j=1}^{M} are updated with self-attention, yielding {sj}j=1Msuperscriptsubscriptsubscript𝑠𝑗𝑗1𝑀\{s_{j}\}_{j=1}^{M}, which are then used in top-down inference for token representations, as discussed in Section 2.2.
...
We use the same encoder-decoder architecture for all datasets. The encoder has 8 bottom-up inference layers and 4 top-down inference layers for tokens, and 2 self-attention layers for segments. The decoder has 12 layers. The encoder layers for tokens (12 layers) and the decoder layers are all initialized from BART (Lewis et al., 2020) except the parameters for token-segment cross-attention in the top-down inference layers, which are randomly initialized. The self-attention parameters for segments are also randomly initialized. The window size is 102410241024 unless otherwise specified. Our settings closely follow Longformer (Beltagy et al., 2020) which has 12 layers for the encoder and decoder, is initialized from BART, and uses a local window size of 102410241024. Thus, comparison with Longformer is a test of the effect of top-down correction for token representations. PubMed, arXiv, and CNN-DailyMail are obtained from Huggingface Datasets 111https://huggingface.co/datasets. SummScreen and BookSum are provided by the authors. Standard train/validation/test splits, provided by either Huggingface or the dataset authors, are used for all datasets. Model performance is evaluated with ROUGE scores (Lin, 2004). Reported performance is based on the checkpoint with the best validation R-2 score. Summary samples for each dataset generated by our models are provided in the appendix.
...
Ours
Top Down Transformer (AvgPool) (464M)
...
Top Down Transformer (OracleAdaPool)
55.15
26.55
50.25
64.16
...
Our model parameters are initialized from BART. Hence, BART with full self-attention forms a natural baseline, allowing for direct comparison. In the bottom-up inference, the local attention window size is 256256256. As shown in Table 3, models under our framework achieve slightly better performance, especially in terms of R-1 and R-L, than BART. It confirms our hypothesis that a synergy of bottom-up inference with local attention and top-down inference with global attention is effective and achieves on-par or better performance as full self-attention.
...
Hybrid (BART + Oracle Content Selection)
42.1
11.9
40.9
26.4
...
##### Inference for Hierarchical Latent Variable Model
...
Natural Language Processing (EMNLP), pp. 268–284, Online, November 2020.
...
for Computational Linguistics (Volume 1: Long Papers), pp. 76–86,
...
for Computational Linguistics (Volume 1: Long Papers), pp. 484–494,
...
R. Child, Scott Gray, Alec Radford, and Ilya Sutskever.

Generating long sequences with sparse transformers.

ArXiv, abs/1904.10509, 2019.
...
Technologies, Volume 2 (Short Papers), pp. 615–621, New Orleans,
...
Sliding selector network with dynamic memory for extractive
summarization of long documents.

In Proceedings of the 2021 Conference of the North American
Chapter of the Association for Computational Linguistics: Human Language
Technologies, pp. 5881–5891, Online, June 2021. Association for
...
Biva: A very deep hierarchy of latent variables for generative
modeling.
...
In Proceedings of the 58th Annual Meeting of the Association
for Computational Linguistics, pp. 7203–7219, Online, July 2020.
Association for Computational Linguistics.
...
Learning multi-layer latent variable model via variational
optimization of short run mcmc for approximate inference.

In European Conference on Computer Vision, pp. 361–378.
Springer, 2020.
...
In Proceedings of the 2020 Conference on Empirical Methods in
Natural Language Processing (EMNLP), pp. 9308–9319, Online, November
...
Chapter of the Association for Computational Linguistics: Human Language
Technologies, Volume 1 (Long and Short Papers), pp. 380–385, Minneapolis,
...
In International Conference on Machine Learning, pp. 10183–10192. PMLR, 2021.
...
Cluster-former: Clustering-based sparse transformer for question
answering.

In Findings of the Association for Computational Linguistics:
ACL-IJCNLP 2021, pp. 3958–3968, Online, August 2021. Association for
Computational Linguistics.
...
Shusheng Xu, Xingxing Zhang, Yi Wu, Furu Wei, and Ming Zhou.

Unsupervised extractive summarization by pre-training hierarchical
transformers.

In Findings of the Association for Computational Linguistics:
EMNLP 2020, pp. 1784–1795, Online, November 2020. Association for
Computational Linguistics.
...
for Computational Linguistics, pp. 5059–5069, Florence, Italy, July 2019.
Association for Computational Linguistics.
...
with top-down update via cross-attention
window size - 1024
48.34
21.40
44.22

with top-down update via concat
window size - 1024
47.04
20.36
43.03

without top-down update
window size - 1024
46.97
20.23
42.88
...
Generated on Mon Mar 11 07:07:02 2024 by LaTeXML
