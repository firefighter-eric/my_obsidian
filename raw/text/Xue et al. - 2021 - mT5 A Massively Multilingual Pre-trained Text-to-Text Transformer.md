...
Linting Xue  Noah Constant11footnotemark: 1  Adam Roberts11footnotemark: 1 
...
In this paper, we introduce mT5, a multilingual variant of T5 that was pre-trained on a new Common Crawl-based dataset covering 101101101 languages.
...
All of the code and model checkpoints used in this work are publicly available.111https://goo.gle/mt5-code

## 1 Introduction

Current natural language processing (NLP) pipelines often make use of transfer learning, where a model is pre-trained on a data-rich task before being fine-tuned on a downstream task of interest (Ruder et al., 2019).
...
As such, mT5 inherits all of the benefits of T5 (described in section 2), such as its general-purpose text-to-text format, its design based on insights from a large-scale empirical study, and its scale.
...
270270270M – 550550550M
100100100
Common Crawl (CCNet)

mBART (Lewis et al., 2020b)
...
The C4 dataset was explicitly designed to be English only: any page that was not given a probability of at least 99%percent9999\% of being English by langdetect222https://pypi.org/project/langdetect/ was discarded.
In contrast, for mC4 we use cld3333https://github.com/google/cld3 to identify over 100100100 languages.
...
Otherwise, we follow C4’s filtering by deduplicating lines across documents and removing pages containing bad words.444https://github.com/LDNOOBW/
...
A histogram of the page counts for each language is shown in fig. 1. Detailed dataset statistics including per-language token counts are shown in the appendix.

### 3.2 mT5

The model architecture and training procedure that we use for mT5 closely follows that of T5.
Specifically, we base mT5 on the ‘‘T5.1.1’’ recipe,555https://github.com/google-research/text-to-text-transfer-transformer/blob/master/released_checkpoints.md#t511 which improves upon T5 by using GeGLU nonlinearities (Shazeer, 2020), scaling both dmodelsubscriptdmodel\mathrm{d_{model}} and dffsubscriptdff\mathrm{d_{ff}} instead of just dffsubscriptdff\mathrm{d_{ff}} in the larger models, and pre-training on unlabeled data only with no dropout.
...
We tried all three of these values (ablation results in section 4.2) and found α=0.3𝛼0.3\alpha=0.3 to give a reasonable compromise between performance on high- and low-resource languages.
...
Table 2 presents our main results, with per-language breakdowns for each task given in the appendix.
...
Figure 2 shows that model capacity is key to improving performance on variants of the TyDi QA GoldP task in the absence of ‘‘gold’’ multilingual data:
...
The results are shown in table 3, with results for T5 reproduced from Raffel et al. (2020).
...
No line length filter
79.1

Add Wikipedia data
80.3

The effect of these ablations on XNLI zero-shot accuracy is shown in table 4.
...
For example, on zero-shot XQuAD, a non-trivial portion of mT5 mistakes are in fact illegal spans, for all model sizes (cf. fig. 4 ‘‘Baseline’’).
Through inspection, we find these illegal predictions mainly fall into three categories: (i) normalization, (ii) grammatical adjustment, and (iii) accidental translation. Table 5 provides examples of each type.
...
This is particularly common in Thai, Chinese and Hindi, where most mT5-XXL illegal predictions are resolved by normalization, as seen in fig. 3(b).
...
For example, mT5-XXL’s Arabic and Russian predictions in the middle rows of table 5 are judged by native speakers as correct and grammatical answers to the posed XQuAD questions, while the gold targets are judged as ungrammatical answers.
...
The problem is most prevalent in mT5-Small and mT5-Base, where from manual inspection, half or more of the illegal predictions within each language exhibit accidental translation, with many of the illegal predictions coming from Greek and Russian, as shown in fig. 3(a).
While we do observe full phrase translations, a more common occurrence is partial translation, where the model outputs a token or two of English before reverting to the correct target language.
...
With these changes, we mix a small amount of our unsupervised task (covering 101 languages) into XQuAD fine-tuning, at a ratio of just 111:100100100. Figure 4 shows the results on XQuAD zero-shot error rates.
...
We release all code and pre-trained datasets used in this paper to facilitate future work on multilingual language understanding.999https://goo.gle/mt5-code
...
Diedre Carmo, Marcos Piau, Israel Campiotti, Rodrigo Nogueira, and Roberto
Lotufo. 2020.

PTT5: Pretraining and validating the t5 model on brazilian
portuguese data.

arXiv preprint arXiv:2008.09144.
...
Hyung Won Chung, Thibault Févry, Henry Tsai, Melvin Johnson, and Sebastian
Ruder. 2020.
...
arXiv preprint arXiv:2005.10433.
...
EMNLP 2020, pages 1896–1907, Online. Association for Computational
Linguistics.
...
Conference, pages 2479–2490, Marseille, France. European Language Resources
Association.

- Lewis et al. (2020a)

Mike Lewis, Marjan Ghazvininejad, Gargi Ghosh, Armen Aghajanyan, Sida Wang, and
Luke Zettlemoyer. 2020a.
...
Ghazvininejad, Mike Lewis, and Luke Zettlemoyer. 2020a.
...
arXiv preprint arXiv:2004.14218.

- Luo et al. (2020)

Fuli Luo, Wei Wang, Jiahao Liu, Yijia Liu, Bin Bi, Songfang Huang, Fei Huang,
and Luo Si. 2020.

Veco: Variable encoder-decoder pre-training for cross-lingual
...
Martin Malmsten, Love Börjeson, and Chris Haffenden. 2020.

Playing with words at the national library of sweden–making a
swedish BERT.
...
In Findings of the Association for Computational Linguistics:
EMNLP 2020, pages 1037–1042, Online. Association for Computational
Linguistics.
...
for Computational Linguistics (Volume 1: Long Papers), pages 1946–1958,
Vancouver, Canada. Association for Computational Linguistics.

- Phang et al. (2020)

Jason Phang, Phu Mon Htut, Yada Pruksachatkun, Haokun Liu, Clara Vania,
Katharina Kann, Iacer Calixto, and Samuel R Bowman. 2020.
...
Natural Language Processing (EMNLP-IJCNLP), pages 3687–3692, Hong Kong,
...
in 1,2,3,4,5
See large_appendix_tables.pdf

Generated on Thu Mar 14 16:55:34 2024 by LaTeXML
