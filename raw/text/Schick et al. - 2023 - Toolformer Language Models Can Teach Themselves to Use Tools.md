...
Our aim is to equip a language model M𝑀M with the ability to use different tools by means of API calls. We require that inputs and outputs for each API can be represented as text sequences.
...
We represent each API call as a tuple c=(ac,ic)𝑐subscript𝑎𝑐subscript𝑖𝑐c=({a}_{c},{i}_{c}) where acsubscript𝑎𝑐a_{c} is the name of the API and icsubscript𝑖𝑐i_{c} is the corresponding input. Given an API call c𝑐c with a corresponding result r𝑟r, we denote the linearized sequences of the API call not including and including its result, respectively, as:

e​(c)e𝑐\displaystyle\text{e}(c)
=<API>​ac​(​ic​)​</API>absent<API>subscript𝑎𝑐(subscript𝑖𝑐)</API>\displaystyle=\texttt{<API>}\,a_{c}\texttt{(}i_{c}\texttt{)}\,\texttt{</API>}

e​(c,r)e𝑐𝑟\displaystyle\text{e}(c,r)
=<API>​ac​(​ic​)→r​</API>absent<API>subscript𝑎𝑐(subscript𝑖𝑐)→𝑟</API>\displaystyle=\texttt{<API>}\,a_{c}\texttt{(}i_{c}\texttt{)}\rightarrow r\,\texttt{</API>}

where “<API>”, “</API>” and “→→\rightarrow” are special tokens.111In practice, we use the token sequences “ [”, “]” and “->” to represent “<API>”, “</API>” and “→→\rightarrow”, respectively. This enables our approach to work without modifying the existing LM’s vocabulary. For reasons of readability, we still refer to them as “<API>”, “</API>” and “→→\rightarrow” throughout this section. Some examples of linearized API calls inserted into text sequences are shown in Figure 1.

Given a dataset 𝒞={𝐱1,…,𝐱|𝒞|}𝒞superscript𝐱1…superscript𝐱𝒞\mathcal{C}=\{\mathbf{x}^{1},\ldots,\mathbf{x}^{|\mathcal{C}|}\} of plain texts, we first convert this dataset into a dataset 𝒞∗superscript𝒞\mathcal{C}^{*} augmented with API calls. This is done in three steps, illustrated in Figure 2: First, we exploit the in-context learning ability of M𝑀M to sample a large number of potential API calls. We then execute these API calls and finally check whether the obtained responses are helpful for predicting future tokens; this is used as a filtering criterion. After filtering, we merge API calls for different tools, resulting in the augmented dataset 𝒞∗superscript𝒞\mathcal{C}^{*}, and finetune M𝑀M itself on this dataset. Each of these steps is described in more detail below.

##### Sampling API Calls

For each API, we write a prompt P​(𝐱)𝑃𝐱P(\mathbf{x}) that encourages the LM to annotate an example 𝐱=x1,…,xn𝐱subscript𝑥1…subscript𝑥𝑛\mathbf{x}=x_{1},\ldots,x_{n} with API calls. An example of such a prompt for a question answering tool is shown in Figure 3; all prompts used are shown in Appendix A.2. Let pM​(zn+1∣z1,…,zn)subscript𝑝𝑀conditionalsubscript𝑧𝑛1subscript𝑧1…subscript𝑧𝑛p_{M}(z_{n+1}\mid z_{1},\ldots,z_{n}) be the probability that M𝑀M assigns to token zn+1subscript𝑧𝑛1z_{n+1} as a continuation for the sequence z1,…,znsubscript𝑧1…subscript𝑧𝑛z_{1},\ldots,z_{n}. We first sample up to k𝑘k candidate positions for doing API calls by computing, for each i∈{1,…,n}𝑖1…𝑛i\in\{1,\ldots,n\}, the probability

pi=pM​(<API>∣P​(𝐱),x1:i−1)subscript𝑝𝑖subscript𝑝𝑀conditional<API>𝑃𝐱subscript𝑥:1𝑖1p_{i}=p_{M}(\texttt{<API>}\mid P(\mathbf{x}),x_{1:i-1})

that M𝑀M assigns to starting an API call at position i𝑖i.
Given a sampling threshold τssubscript𝜏𝑠\tau_{s}, we keep all positions I={i∣pi>τs}𝐼conditional-set𝑖subscript𝑝𝑖subscript𝜏𝑠I=\{i\mid p_{i}>\tau_{s}\}; if there are more than k𝑘k such positions, we only keep the top k𝑘k.

For each position i∈I𝑖𝐼i\in I, we then obtain up to m𝑚m API calls ci1,…,cimsuperscriptsubscript𝑐𝑖1…superscriptsubscript𝑐𝑖𝑚c_{i}^{1},\ldots,c_{i}^{m} by sampling from M𝑀M given the sequence [P​(𝐱),x1,…,xi−1,<API>]𝑃𝐱subscript𝑥1…subscript𝑥𝑖1<API>[P(\mathbf{x}),x_{1},\ldots,x_{i-1},\texttt{<API>}] as a prefix and </API> as an end-of-sequence token.222We discard all examples where M𝑀M does not generate the </API> token.

##### Executing API Calls

As a next step, we execute all API calls generated by M𝑀M to obtain the corresponding results. How this is done depends entirely on the API itself – for example, it can involve calling another neural network, executing a Python script or using a retrieval system to perform search over a large corpus. The response for each API call cisubscript𝑐𝑖c_{i} needs to be a single text sequence risubscript𝑟𝑖r_{i}.
...
After sampling and filtering calls for all APIs, we finally merge the remaining API calls and interleave them with the original inputs. That is, for an input text 𝐱=x1,…,xn𝐱subscript𝑥1…subscript𝑥𝑛\mathbf{x}=x_{1},\ldots,x_{n} with a corresponding API call and result (ci,ri)subscript𝑐𝑖subscript𝑟𝑖(c_{i},r_{i}) at position i𝑖i, we construct the new sequence 𝐱∗=x1:i−1,e​(ci,ri),xi:nsuperscript𝐱subscript𝑥:1𝑖1esubscript𝑐𝑖subscript𝑟𝑖subscript𝑥:𝑖𝑛\mathbf{x}^{*}=x_{1:{i-1}},\text{e}(c_{i},r_{i}),x_{i:n}; we proceed analogously for texts with multiple API calls. Doing this for all 𝐱∈𝒞𝐱𝒞\mathbf{x}\in\mathcal{C} results in the new dataset 𝒞∗superscript𝒞\mathcal{C}^{*} augmented with API calls. We use this new dataset to finetune M𝑀M, using a standard language modeling objective. Crucially, apart from inserted API calls the augmented dataset 𝒞∗superscript𝒞\mathcal{C}^{*} contains the exact same texts as 𝒞𝒞\mathcal{C}, the original dataset. As a consequence, finetuning M𝑀M on 𝒞∗superscript𝒞\mathcal{C}^{*} exposes it to the same content as finetuning on 𝒞𝒞\mathcal{C}. Moreover, as API calls are inserted in exactly those positions and with exactly those inputs that help M𝑀M predict future tokens, finetuning on 𝒞∗superscript𝒞\mathcal{C}^{*} enables the language model to decide when and how to use which tool, based purely on its own feedback.

##### Inference

When generating text with M𝑀M after finetuning with our approach, we perform regular decoding until M𝑀M produces the “→→\rightarrow” token, indicating that it next expects the response for an API call. At this point, we interrupt the decoding process, call the appropriate API to get a response, and continue the decoding process after inserting both the response and the </API> token.
...
Where was the Knights of Columbus founded?
...
Throughout all of our experiments, we use a subset of CCNet (Wenzek et al., 2020) as our language modeling dataset 𝒞𝒞\mathcal{C} and GPT-J (Wang and Komatsuzaki, 2021) as our language model M𝑀M. To reduce the computational cost of annotating 𝒞𝒞\mathcal{C} with API calls, we define heuristics for some APIs to get a subset of 𝒞𝒞\mathcal{C} for which API calls are more likely to be helpful than for an average text. For example, we only consider texts for the calculator tool if they contain at least three numbers. Details of the heuristics used are given in Appendix A. For obtaining 𝒞∗superscript𝒞\mathcal{C}^{*} from 𝒞𝒞\mathcal{C}, we perform all steps described in Section 2 and additionally filter out all examples for which all API calls were eliminated in the filtering step.444While this filtering alters the distribution of training examples, we assume that the remaining examples are close enough to the original distribution so that M𝑀M’s language modeling abilities remain unaffected. This assumption is empirically validated in Section 4.3. For the weighting function, we use

wt=w~t∑s∈ℕw~s​ with ​w~t=max⁡(0,1−0.2⋅t)subscript𝑤𝑡subscript~𝑤𝑡subscript𝑠ℕsubscript~𝑤𝑠 with subscript~𝑤𝑡01⋅0.2𝑡w_{t}=\frac{\tilde{w}_{t}}{\sum_{s\in\mathbb{N}}\tilde{w}_{s}}\text{ with }\tilde{w}_{t}=\max(0,1-0.2\cdot t)

to make sure that API calls happen close to where the information provided by the API is actually helpful for the model. The thresholds τssubscript𝜏𝑠\tau_{s} and τfsubscript𝜏𝑓\tau_{f} are chosen individually for each tool to ensure a sufficiently larger number of examples; see Appendix A for details.
Table 2 shows relevant statistics of our final dataset augmented with API calls.

Number of Examples

API

τf=0.5subscript𝜏𝑓0.5\tau_{f}=0.5
τf=1.0subscript𝜏𝑓1.0\tau_{f}=1.0
τf=2.0subscript𝜏𝑓2.0\tau_{f}=2.0
...
We finetune M𝑀M on 𝒞∗superscript𝒞\mathcal{C}^{*} using a batch size of 128 and a learning rate of 1⋅10−5⋅1superscript1051\cdot 10^{-5} with linear warmup for the first 10% of training. Details of our finetuning procedure are given in Appendix B.
...
GPT-J + CC: GPT-J finetuned on 𝒞𝒞\mathcal{C}, our subset of CCNet without any API calls.

- •

Toolformer: GPT-J finetuned on 𝒞∗superscript𝒞\mathcal{C}^{*}, our subset of CCNet augmented with API calls.
...
For most tasks, we additionally compare to OPT (66B) (Zhang et al., 2022) and GPT-3666We use the original davinci variant that is not finetuned on any instructions. (175B) (Brown et al., 2020), two models that are about 10 and 25 times larger than our other baseline models, respectively.
...
Li−−Li+superscriptsubscript𝐿𝑖superscriptsubscript𝐿𝑖L_{i}^{-}\,{-}\,L_{i}^{+}
Useful
...
Note: The WL will be open on Friday, <API> Calendar() →→\rightarrow Today is Thursday, March 9, 2017. </API> March 10, and Sunday, March 19 for regular hours.

2.11
✓

The Nile has an approximate length of <API> QA(What is the approximate length of the Nile?) →→\rightarrow 6,853 km </API> 6,853 kilometers, the White Nile being its main source.

2.08
✓

If Venus had an atmosphere similar to Earth’s then you would expect Venus’ mean temperature to be 499 K (1.74 x 287) rather than 735 K which is <API> Calculator(735 / 499) →→\rightarrow 1.47 </API> 1.47 (735 / 499) times hotter than it should be.

1.59
✓

You are here: Home / Featured / Catch this fast train to success! <API> WikiSearch(Fast train success) →→\rightarrow Fast Train ¿ It also peaked at #23 on the Canadian CHUM singles chart, on June 26, 1971. The success of this single established Myles Goodwyn as the band’s main songwriter, and made it possible for April Wine to record a second album. </API> Don’t wait weeks and […]delimited-[]…\left[\ldots\right]

0.92
✗

Os Melhores Escolas em Jersey 2020 <API> MT(Os Melhores Escolas em Jersey) →→\rightarrow The Best Schools in Jersey </API> On this page you can search for Universities, Colleges and Business schools in Jersey

0.70
✓

Enjoy these pictures from the <API> Calendar() →→\rightarrow Today is Friday, April 19, 2013. </API> Easter Egg Hunt.

0.33
✓

85 patients (23%) were hospitalised alive and admitted to a hospital ward. Of them, <API> Calculator(85 / 23) →→\rightarrow 3.70 </API> 65% had a cardiac aetiology […]delimited-[]…\left[\ldots\right]

−-0.02−-

✗

But hey, after the <API> Calendar() →→\rightarrow Today is Saturday, June 25, 2011. </API> Disneyland fiasco with the fire drill, I think it’s safe to say Chewey won’t let anyone die in a fire.

−-0.41−-

✗

The last time I was with <API> QA(Who was last time I was with?) →→\rightarrow The Last Time </API> him I asked what he likes about me and he said he would tell me one day.
...
The idea of using self-training and bootstrapping techniques to improve models has been investigated in various contexts, ranging from word sense disambiguation (Yarowsky, 1995), relation extraction (Brin, 1999; Agichtein and Gravano, 2000), parsing (McClosky et al., 2006; Reichart and Rappoport, 2007), sequence generation (He et al., 2020), few-shot text classification (Schick and
Schütze, 2021a) and retrieval (Izacard and Grave, 2021) to reasoning (Zelikman et al., 2022). In a similar spirit to these approaches, Toolformer is trained on its own predictions after applying a perplexity-based filtering step.
...
In The World Wide Web and Databases, pages 172–183, Berlin,
...
Francisco Guzmán, Armand Joulin, and Edouard Grave. 2020.
...
When sampling and filtering API calls, by default we use values of τs=0.05subscript𝜏𝑠0.05\tau_{s}=0.05 and τf=1.0subscript𝜏𝑓1.0\tau_{f}=1.0 – i.e., we only make API calls at positions where the probability of the <API> token is at least 5%, and we keep API calls if they reduce the loss by at least 1.0. We only keep the top k=5𝑘5k=5 such positions and sample up to m=5𝑚5m=5 API calls for each position identified in a piece of text. Due to the heuristic filtering described below, we generate API calls for the calculator and machine translation system on only a small subset of 𝒞𝒞\mathcal{C}; to compensate for this, we set τs=0.0subscript𝜏𝑠0.0\tau_{s}=0.0, k=20𝑘20k=20 and m=10𝑚10m=10 for these tools. As the resulting sets of API calls are still comparably small, we additionally set τf=0.5subscript𝜏𝑓0.5\tau_{f}=0.5.

### A.1 Implementation

##### Question Answering

We use the Atlas model of Izacard et al. (2022) finetuned on Natural Questions (Kwiatkowski et al., 2019) as our question answering system. For creating 𝒞∗superscript𝒞\mathcal{C}^{*} we use Atlas-large, enabling us to efficiently process millions of API calls; during inference, we use the larger Atlas-xxl model.
...
For both LAMA and TempLAMA, given an input text 𝐱𝐱\mathbf{x}, we use the following prompt: Please complete the following text so that it is factually correct: 𝐱𝐱\mathbf{x}.

### C.2 Math Benchmarks

For all math benchmarks, given a context 𝐱𝐱\mathbf{x} and a question 𝐪𝐪\mathbf{q}, our prompt is: 𝐱​𝐪𝐱𝐪\mathbf{x}\ \mathbf{q} The answer is.
...
For MLQA, given a context 𝐱𝐱\mathbf{x} and a question 𝐪𝐪\mathbf{q}, our prompt is: Your task is to answer a question based on the following paragraph: 𝐱𝐱\mathbf{x} Now answer the following question in English: 𝐪𝐪\mathbf{q}.
...
How many days {ago was, are there until} {past_date, future_date}?

00,400

What {day of the week, day of the month, month, year} was it (current_date – past_date) {days, weeks, months, years} ago?

00,800

What {day of the week, day of the month, month, year} will it be in (future_date – current_date) days?

00,800

What day of the week {is, was} it on {past_date, future_date}?

00,400

What {day of the week, day of the month, month, year} {is, was} it {the day before yesterday, yesterday, today, tomorrow, the day after tomorrow}?

04,000

What {day of the week, day of the month, month} {is, was} h​o​l​i​d​a​yℎ𝑜𝑙𝑖𝑑𝑎𝑦holiday this year?

01,800

How many {days, weeks, months, years} {ago was, are there until} h​o​l​i​d​a​yℎ𝑜𝑙𝑖𝑑𝑎𝑦holiday this year?
...
Generated on Fri Mar 1 02:55:09 2024 by LaTeXML
