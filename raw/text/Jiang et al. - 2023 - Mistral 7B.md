...
We introduce Mistral 7B, a 7–billion-parameter language model engineered for superior performance and efficiency.
...
This release is accompanied by a reference implementation111https://github.com/mistralai/mistral-src facilitating easy deployment either locally or on cloud platforms such as AWS, GCP, or Azure using the vLLM kwon2023efficient inference server and SkyPilot 222https://github.com/skypilot-org/skypilot.
Integration with Hugging Face 333https://huggingface.co/mistralai is also streamlined for easier integration.
...
The hidden state in position i𝑖i of the layer k𝑘k, hisubscriptℎ𝑖h_{i}, attends to all hidden states from the previous layer with positions between i−W𝑖𝑊i-W and i𝑖i.
Recursively, hisubscriptℎ𝑖h_{i} can access tokens from the input layer at a distance of up to W×k𝑊𝑘W\times k tokens, as illustrated in Figure 1.
...
Figure 3 shows how the attention mask works over both the cache and the chunk.
...
Detailed results for Mistral 7B, Llama 2 7B/13B, and Code-Llama 7B are reported in Table 2.
Figure 4 compares the performance of Mistral 7B with Llama 2 7B/13B, and Llama 1 34B444Since Llama 2 34B was not open-sourced, we report results for Llama 1 34B. in different categories.
...
6.84 ±plus-or-minus\pm 0.07

Llama 2 system prompt

6.38 ±plus-or-minus\pm 0.07

Mistral system prompt

6.58 ±plus-or-minus\pm 0.05
...
Generated on Wed Feb 28 01:53:44 2024 by LaTeXML
