...
The Transformer uses constant latent vector size D𝐷D through all of its layers, so we flatten the patches and map to D𝐷D dimensions with a trainable linear projection (Eq. 1).
...
𝐳0subscript𝐳0\displaystyle\mathbf{z}_{0}
=[𝐱class;𝐱p1​𝐄;𝐱p2​𝐄;⋯;𝐱pN​𝐄]+𝐄p​o​s,absentsubscript𝐱classsubscriptsuperscript𝐱1𝑝𝐄subscriptsuperscript𝐱2𝑝𝐄⋯subscriptsuperscript𝐱𝑁𝑝𝐄subscript𝐄𝑝𝑜𝑠\displaystyle=[\mathbf{x}_{\text{class}};\,\mathbf{x}^{1}_{p}\mathbf{E};\,\mathbf{x}^{2}_{p}\mathbf{E};\cdots;\,\mathbf{x}^{N}_{p}\mathbf{E}]+\mathbf{E}_{pos},
𝐄∈ℝ(P2⋅C)×D,𝐄p​o​s∈ℝ(N+1)×Dformulae-sequence𝐄superscriptℝ⋅superscript𝑃2𝐶𝐷subscript𝐄𝑝𝑜𝑠superscriptℝ𝑁1𝐷\displaystyle\mathbf{E}\in\mathbb{R}^{(P^{2}\cdot C)\times D},\,\mathbf{E}_{pos}\in\mathbb{R}^{(N+1)\times D}

(1)

𝐳′ℓsubscriptsuperscript𝐳′ℓ\displaystyle\mathbf{z^{\prime}}_{\ell}
=MSA⁡(LN⁡(𝐳ℓ−1))+𝐳ℓ−1,absentMSALNsubscript𝐳ℓ1subscript𝐳ℓ1\displaystyle=\operatorname{MSA}(\operatorname{LN}(\mathbf{z}_{\ell-1}))+\mathbf{z}_{\ell-1},
ℓ=1​…​Lℓ1…𝐿\displaystyle\ell=1\ldots L

(2)

𝐳ℓsubscript𝐳ℓ\displaystyle\mathbf{z}_{\ell}
=MLP⁡(LN⁡(𝐳′ℓ))+𝐳′ℓ,absentMLPLNsubscriptsuperscript𝐳′ℓsubscriptsuperscript𝐳′ℓ\displaystyle=\operatorname{MLP}(\operatorname{LN}(\mathbf{z^{\prime}}_{\ell}))+\mathbf{z^{\prime}}_{\ell},
ℓ=1​…​Lℓ1…𝐿\displaystyle\ell=1\ldots L

(3)

𝐲𝐲\displaystyle\mathbf{y}
=LN⁡(𝐳L0)absentLNsuperscriptsubscript𝐳𝐿0\displaystyle=\operatorname{LN}(\mathbf{z}_{L}^{0})
...
In this hybrid model, the patch embedding projection 𝐄𝐄\mathbf{E} (Eq. 1) is applied to patches extracted from a CNN feature map.
...
We train all models, including ResNets, using Adam (Kingma & Ba, 2015) with β1=0.9subscript𝛽10.9\beta_{1}=0.9, β2=0.999subscript𝛽20.999\beta_{2}=0.999, a batch size of 4096 and apply a high weight decay of 0.10.10.1, which we found to be useful for transfer of all models (Appendix D.1 shows that, in contrast to common practices, Adam works slightly better than SGD for ResNets in our setting).
...
For ImageNet results in Table 2, we fine-tuned at higher resolution: 512512512 for ViT-L/16 and 518518518 for ViT-H/14, and also used Polyak & Juditsky (1992) averaging with a factor of 0.99990.99990.9999 (Ramachandran et al., 2019; Wang et al., 2020b).
...
ImageNet
88.55± 0.04plus-or-minus88.550.04\mathbf{88.55}{\scriptstyle\,\pm\,0.04}
87.76± 0.03plus-or-minus87.760.0387.76{\scriptstyle\,\pm\,0.03}
85.30± 0.02plus-or-minus85.300.0285.30{\scriptstyle\,\pm\,0.02}
87.54± 0.02plus-or-minus87.540.0287.54{\scriptstyle\,\pm\,0.02}
88.4/88.5∗88.4superscript88.588.4/88.5^{*}

ImageNet ReaL
90.72± 0.05plus-or-minus90.720.05\mathbf{90.72}{\scriptstyle\,\pm\,0.05}
90.54± 0.03plus-or-minus90.540.0390.54{\scriptstyle\,\pm\,0.03}
88.62± 0.05plus-or-minus88.620.0588.62{\scriptstyle\,\pm\,0.05}
90.5490.5490.54
90.5590.5590.55

CIFAR-10
99.50± 0.06plus-or-minus99.500.06\mathbf{99.50}{\scriptstyle\,\pm\,0.06}
99.42± 0.03plus-or-minus99.420.0399.42{\scriptstyle\,\pm\,0.03}
99.15± 0.03plus-or-minus99.150.0399.15{\scriptstyle\,\pm\,0.03}
99.37± 0.06plus-or-minus99.370.0699.37{\scriptstyle\,\pm\,0.06}
−-

CIFAR-100
94.55± 0.04plus-or-minus94.550.04\mathbf{94.55}{\scriptstyle\,\pm\,0.04}
93.90± 0.05plus-or-minus93.900.0593.90{\scriptstyle\,\pm\,0.05}
93.25± 0.05plus-or-minus93.250.0593.25{\scriptstyle\,\pm\,0.05}
93.51± 0.08plus-or-minus93.510.0893.51{\scriptstyle\,\pm\,0.08}
−-

Oxford-IIIT Pets
97.56± 0.03plus-or-minus97.560.03\mathbf{97.56}{\scriptstyle\,\pm\,0.03}
97.32± 0.11plus-or-minus97.320.1197.32{\scriptstyle\,\pm\,0.11}
94.67± 0.15plus-or-minus94.670.1594.67{\scriptstyle\,\pm\,0.15}
96.62± 0.23plus-or-minus96.620.2396.62{\scriptstyle\,\pm\,0.23}
−-

Oxford Flowers-102
99.68± 0.02plus-or-minus99.680.0299.68{\scriptstyle\,\pm\,0.02}
99.74± 0.00plus-or-minus99.740.00\mathbf{99.74}{\scriptstyle\,\pm\,0.00}
99.61± 0.02plus-or-minus99.610.0299.61{\scriptstyle\,\pm\,0.02}
99.63± 0.03plus-or-minus99.630.0399.63{\scriptstyle\,\pm\,0.03}
−-

VTAB (19 tasks)
77.63± 0.23plus-or-minus77.630.23\mathbf{77.63}{\scriptstyle\,\pm\,0.23}
76.28± 0.46plus-or-minus76.280.4676.28{\scriptstyle\,\pm\,0.46}
72.72± 0.21plus-or-minus72.720.2172.72{\scriptstyle\,\pm\,0.21}
76.29± 1.70plus-or-minus76.291.7076.29{\scriptstyle\,\pm\,1.70}
−-
...
Figure 4 shows the results after fine-tuning to ImageNet (results on other datasets are shown in Table 5)222Note that the ImageNet pre-trained models are also fine-tuned, but again on ImageNet. This is because the resolution increase during fine-tuning improves the performance..
...
I. Bello, B. Zoph, Q. Le, A. Vaswani, and J. Shlens.
...
Dan Moldovan, Sylvan Gelly, Neil Houlsby, Xiaohua Zhai, and Mario Lucic.
...
Zilong Huang, Xinggang Wang, Yunchao Wei, Lichao Huang, Humphrey Shi, Wenyu
Liu, and Thomas S. Huang.
...
arXiv preprint arXiv:2003.08237, 2020.

- Tschannen et al. (2020)

Michael Tschannen, Josip Djolonga, Marvin Ritter, Aravindh Mahendran, Neil
Houlsby, Sylvain Gelly, and Mario Lucic.
...
arXiv preprint arXiv:2003.07853, 2020b.
...
Standard 𝐪𝐤𝐯𝐪𝐤𝐯\mathbf{qkv} self-attention (SA, Vaswani et al. (2017)) is a popular building block for neural architectures. For each element in an input sequence 𝐳∈ℝN×D𝐳superscriptℝ𝑁𝐷\mathbf{z}\in\mathbb{R}^{N\times D}, we compute a weighted sum over all values 𝐯𝐯\mathbf{v} in the sequence. The attention weights Ai​jsubscript𝐴𝑖𝑗A_{ij} are based on the pairwise similarity between two elements of the sequence and their respective query 𝐪isuperscript𝐪𝑖\mathbf{q}^{i} and key 𝐤jsuperscript𝐤𝑗\mathbf{k}^{j} representations.

[𝐪,𝐤,𝐯]𝐪𝐤𝐯\displaystyle[\mathbf{q},\mathbf{k},\mathbf{v}]
=𝐳𝐔q​k​vabsentsubscript𝐳𝐔𝑞𝑘𝑣\displaystyle=\mathbf{z}\mathbf{U}_{qkv}
𝐔q​k​vsubscript𝐔𝑞𝑘𝑣\displaystyle\mathbf{U}_{qkv}
∈ℝD×3​Dh,absentsuperscriptℝ𝐷3subscript𝐷ℎ\displaystyle\in\mathbb{R}^{D\times 3D_{h}},

(5)

A𝐴\displaystyle A
=softmax⁡(𝐪𝐤⊤/Dh)absentsoftmaxsuperscript𝐪𝐤topsubscript𝐷ℎ\displaystyle=\operatorname{softmax}\left(\mathbf{q}\mathbf{k}^{\top}/\sqrt{D_{h}}\right)
A𝐴\displaystyle A
∈ℝN×N,absentsuperscriptℝ𝑁𝑁\displaystyle\in\mathbb{R}^{N\times N},

(6)

SA⁡(𝐳)SA𝐳\displaystyle\operatorname{SA}(\mathbf{z})
=A​𝐯.absent𝐴𝐯\displaystyle=A\mathbf{v}\,.

(7)

Multihead self-attention (MSA) is an extension of SA in which we run k𝑘k self-attention operations, called “heads”, in parallel, and project their concatenated outputs. To keep compute and number of parameters constant when changing k𝑘k, Dhsubscript𝐷ℎD_{h} (Eq. 5) is typically set to D/k𝐷𝑘D/k.

MSA⁡(𝐳)MSA𝐳\displaystyle\operatorname{MSA}(\mathbf{z})
=[SA1⁡(z);SA2⁡(z);⋯;SAk⁡(z)]​𝐔m​s​aabsentsubscriptSA1𝑧subscriptSA2𝑧⋯subscriptSA𝑘𝑧subscript𝐔𝑚𝑠𝑎\displaystyle=[\operatorname{SA}_{1}(z);\operatorname{SA}_{2}(z);\cdots;\operatorname{SA}_{k}(z)]\,\mathbf{U}_{msa}
𝐔m​s​a∈ℝk⋅Dh×Dsubscript𝐔𝑚𝑠𝑎superscriptℝ⋅𝑘subscript𝐷ℎ𝐷\displaystyle\mathbf{U}_{msa}\in\mathbb{R}^{k\cdot D_{h}\times D}
...
JFT-300M
7
8⋅10−4⋅8superscript1048\cdot 10^{-4}
linear
0.1
0.0
...
JFT-300M
7
6⋅10−4⋅6superscript1046\cdot 10^{-4}
linear
0.1
0.0
...
JFT-300M
7/14
4⋅10−4⋅4superscript1044\cdot 10^{-4}
linear
0.1
0.0
...
JFT-300M
7
2⋅10−4⋅2superscript1042\cdot 10^{-4}
linear
0.1
0.0
...
ImageNet
300
3⋅10−3⋅3superscript1033\cdot 10^{-3}
cosine
0.3
0.1
...
We run a small grid search over learning rates, see learning rate ranges in Table 4. To do so, we use small sub-splits from the training set (10% for Pets and Flowers, 2% for CIFAR, 1% ImageNet) as development set and train on the remaining data. For final results we train on the entire training set and evaluate on the respective test data.
For fine-tuning ResNets and hybrid models we use the exact same setup, with the only exception of ImageNet where we add another value 0.060.060.06 to the learning rate sweep.
Additionally, for ResNets we also run the setup of Kolesnikov et al. (2020) and select the best results across this run and our sweep.
...
We use a learning rate of 0.010.010.01 and train for 250025002500 steps (Tab. 4).
...
Instead we find that Vision Transformer benefits most from a high resolution (384×384384384384\times 384) for all tasks.
...
Table 5 corresponds to Figure 4 from the paper and shows transfer performance of different ViT models pre-trained on datasets of increasing size: ImageNet, ImageNet-21k, and JFT-300M.
Table 6 corresponds to Figure 5 from the paper and shows the transfer performance of ViT, ResNet, and hybrid models of varying size, as well as the estimated computational cost of their pre-training.
...
Note that the absolute numbers are lower than those reported by Kolesnikov et al. (2020), since we pre-train only for 777 epochs, not 303030.
...
Along this line, Wang et al. (2020b) proposed the AxialResNet model in which all the convolutions with kernel size 3×3333\times 3 in a ResNet50 are replaced by axial self-attention, i.e. a row and column attention, augmented by relative positional encoding.
We have implemented AxialResNet as a baseline model.333Our implementation is based on the open-sourced PyTorch implementation in https://github.com/csrhddlam/axial-deeplab. In our experiments, we reproduced the scores reported in (Wang et al., 2020b) in terms of accuracy, however, our implementation, similar to the open-source implementation, is very slow on TPUs.
...
Generated on Tue Mar 19 05:26:01 2024 by LaTeXML
