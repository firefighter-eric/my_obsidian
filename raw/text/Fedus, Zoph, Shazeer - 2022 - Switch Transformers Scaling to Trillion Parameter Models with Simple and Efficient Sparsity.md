...
We design models based off T5-Base and T5-Large (Raffel et al., 2019) to obtain up to 7x increases in pre-training speed with the same computational resources.
...
Finally, we advance the current scale of language models by pre-training up to trillion parameter models on the “Colossal Clean Crawled Corpus”, and achieve a 4x speedup over the T5-XXL model.111JAX code for Switch Transformer and all model checkpoints are available at https://github.com/google-research/t5x222Tensorflow code for Switch Transformer is available at https://github.com/tensorflow/mesh/blob/master/mesh_tensorflow/transformer/moe.py
...
Shazeer et al. (2017) proposed a natural language Mixture-of-Experts (MoE) layer which takes as an input a token representation x𝑥x and then routes this to the best determined top-k𝑘k experts, selected from a set {Ei​(x)}i=1Nsuperscriptsubscriptsubscript𝐸𝑖𝑥𝑖1𝑁\{E_{i}(x)\}_{i=1}^{N} of N𝑁N experts.
The router variable Wrsubscript𝑊𝑟W_{r} produces logits h​(x)=Wr⋅xℎ𝑥⋅subscript𝑊𝑟𝑥h(x)=W_{r}\cdot x which are normalized via a softmax distribution over the available N𝑁N experts at that layer. The gate-value for expert i𝑖i is given by,

pi​(x)=eh​(x)i∑jNeh​(x)j.subscript𝑝𝑖𝑥superscript𝑒ℎsubscript𝑥𝑖superscriptsubscript𝑗𝑁superscript𝑒ℎsubscript𝑥𝑗p_{i}(x)=\frac{e^{h(x)_{i}}}{\sum_{j}^{N}e^{h(x)_{j}}}.
...
y=∑i∈𝒯pi​(x)​Ei​(x).𝑦subscript𝑖𝒯subscript𝑝𝑖𝑥subscript𝐸𝑖𝑥y=\sum_{i\in\mathcal{T}}p_{i}(x)E_{i}(x).

(2)

Switch Routing: Rethinking Mixture-of-Experts.
Shazeer et al. (2017) conjectured that routing to k>1𝑘1k>1 experts was necessary in order to have non-trivial gradients to the routing functions.
...
Note that for both MoE and Switch Routing, the gate value pi​(x)subscript𝑝𝑖𝑥p_{i}(x) in Equation 2 permits differentiability of the router.
...
(2) The batch size (expert capacity) of each expert can be at least halved since each token is only being routed to a single expert.333See Section 2.2 for a technical description.
...
expert capacity=(tokens per batchnumber of experts)×capacity factor.\text{expert capacity}=\biggr{(}\frac{\text{tokens per batch}}{\text{number of experts}}\biggr{)}\times\text{capacity factor}.
...
This trade-off is explained in Figure 3.
...
We study the impact that these design decisions have on model quality and speed in Table 1.
...
loss=α⋅N⋅∑i=1Nfi⋅Piloss⋅𝛼𝑁superscriptsubscript𝑖1𝑁⋅subscript𝑓𝑖subscript𝑃𝑖\text{loss}=\alpha\cdot N\cdot\sum_{i=1}^{N}f_{i}\cdot P_{i}

(4)

where fisubscript𝑓𝑖f_{i} is the fraction of tokens dispatched to expert i𝑖i,

fi=1T​∑x∈ℬ𝟙​{argmax​p​(x)=i}subscript𝑓𝑖1𝑇subscript𝑥ℬ1argmax𝑝𝑥𝑖f_{i}=\frac{1}{T}\sum_{x\in\mathcal{B}}\mathbbm{1}\{\text{argmax}\>p(x)=i\}

(5)

and Pisubscript𝑃𝑖P_{i} is the fraction of the router probability allocated for expert i𝑖i,
222A potential source of confusion: pi​(x)subscript𝑝𝑖𝑥p_{i}(x) is the probability of routing token x𝑥x to expert i𝑖i. Pisubscript𝑃𝑖P_{i} is the probability fraction to expert i𝑖i across all tokens in the batch ℬℬ\mathcal{B}.

Pi=1T​∑x∈ℬpi​(x).subscript𝑃𝑖1𝑇subscript𝑥ℬsubscript𝑝𝑖𝑥P_{i}=\frac{1}{T}\sum_{x\in\mathcal{B}}p_{i}(x).
...
The auxiliary loss of Equation 4 encourages uniform routing since it is minimized under a uniform distribution. The objective can also be differentiated as the P𝑃P-vector is differentiable, but the f𝑓f-vector is not.
The final loss is multiplied by expert count N𝑁N to keep the loss constant as the number of experts varies since under uniform routing ∑i=1N(fi⋅Pi)=∑i=1N(1N⋅1N)=1Nsuperscriptsubscript𝑖1𝑁⋅subscript𝑓𝑖subscript𝑃𝑖superscriptsubscript𝑖1𝑁⋅1𝑁1𝑁1𝑁\sum_{i=1}^{N}(f_{i}\cdot P_{i})=\sum_{i=1}^{N}(\frac{1}{N}\cdot\frac{1}{N})=\frac{1}{N}.
...
We highlight three key findings from Table 1: (1) Switch Transformers outperform both carefully tuned dense models and MoE Transformers on a speed-quality basis.
...
Table 2 shows that our approach permits nearly equal speed to bfloat16 training while conferring the training stability of float32.
...
(precision)
(Neg. Log Perp.) (↑↑\uparrow)
(Examples/sec) (↑↑\uparrow)

Switch-Base (float32)
-1.718
1160

Switch-Base (bfloat16)
-3.780 [diverged]
...
The router function takes the tokens as input and produces the dispatch and combine tensors used for the selection and recombination of expert computation (refer to Code Block 15 in the Appendix for details).
...
Table 3 measures the improvement of the model quality and reduction of the variance early in training.
...
In addition, consistent with the findings of Kaplan et al. (2020), we find that larger models are also more sample efficient—learning more quickly for a fixed number of observed tokens.

### 3.2 Scaling Results on a Time-Basis

Figure 4 demonstrates that on a step basis, as we increase the number of experts, the performance consistently improves.
...
Figures 5 and 6 address this question.
Figure 5 measures the pre-training model quality as a function of time.
...
Figure 6 considers a different scenario: what if we instead had allocated our resources to a larger dense model?
...
Section 3 demonstrated the superior scaling properties while pre-training, but we now validate that these gains translate to improved language learning abilities on downstream tasks.
...
For both versions, we design a FLOP-matched Switch Transformer, with many more parameters, which is summarized in Table 9.777FLOPS are calculated for the forward pass as done in Kaplan et al. (2020).
...
We then fine-tune across a diverse set of tasks using a dropout rate of 0.1 for all layers except the Switch layers, which use a dropout rate of 0.4 (see Table 4).
...
Using our best distillation technique described in Table 6, we distill a wide variety of sparse models into dense models.
...
Pre-trained Neg. Log Perp. (↑↑\uparrow)
-1.636
-1.505
-1.474
-1.444
...
Distilled Neg. Log Perp. (↑↑\uparrow)
—
-1.587
-1.585
-1.579
...
Table 8 shows results of distilling a 7.4B parameter Switch-Base model, fine-tuned on the SuperGLUE task, into the 223M T5-Base.
...
In Figure 7 we plot the quality improvement in negative log perplexity for all languages of a FLOP-matched Switch model, mSwitch-Base to the T5 base variant, mT5-Base.
...
In Figure 8, we present a different view and now histogram the per step speed-up of using Switch Transformer over the mT5-Base.999The speedup on a step basis is computed as the ratio of the number of steps for the baseline divided by the number of steps required by our model to reach that same quality.
...
The common way to scale a Transformer is to increase dimensions in tandem, like dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model} or df​fsubscript𝑑𝑓𝑓d_{ff}.
...
We assume B𝐵B tokens in the batch, each of dimension dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}.
Both the input (x𝑥x) and output (y𝑦y) of the FFN are of size [B𝐵B, dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}] and the intermediate (hℎh) is of size [B𝐵B, df​fsubscript𝑑𝑓𝑓d_{ff}] where df​fsubscript𝑑𝑓𝑓d_{ff} is typically several times larger than dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}.
In the FFN, the intermediate is h=x​Wi​nℎ𝑥subscript𝑊𝑖𝑛h=xW_{in} and then the output of the layer is y=R​e​L​U​(h)​Wo​u​t𝑦𝑅𝑒𝐿𝑈ℎsubscript𝑊𝑜𝑢𝑡y=ReLU(h)W_{out}.
Thus Wi​nsubscript𝑊𝑖𝑛W_{in} and Wo​u​tsubscript𝑊𝑜𝑢𝑡W_{out} are applied independently to each token and have sizes [dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}, df​fsubscript𝑑𝑓𝑓d_{ff}] and [df​fsubscript𝑑𝑓𝑓d_{ff}, dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}].

We describe two aspects of partitioning: how the weights and batches of data divide over cores, depicted in Figure 9.
...
Tensors and variables with df​fsubscript𝑑𝑓𝑓d_{ff} are then sharded across m𝑚m model-parallel cores.
...
This corresponds to the left-most column of Figure 9.
...
Each core sends a tensor of [B𝐵B, dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}] to compute the second matrix multiplication R​e​L​U​(h)​Wo​u​t𝑅𝑒𝐿𝑈ℎsubscript𝑊𝑜𝑢𝑡ReLU(h)W_{out} because the df​fsubscript𝑑𝑓𝑓d_{ff} dimension is partitioned and must be summed over.
...
With a total of N=n×m𝑁𝑛𝑚N=n\times m cores, now each core will be responsible for B/n𝐵𝑛B/n tokens and df​f/msubscript𝑑𝑓𝑓𝑚d_{ff}/m of both the weights and intermediate activation.
In the forward and backward pass each core communicates a tensor of size [B/n,dm​o​d​e​l]𝐵𝑛subscript𝑑𝑚𝑜𝑑𝑒𝑙[B/n,d_{model}] in an all-reduce operation.
...
This binary matrix is then used to do a gather via matrix multiplication with the input tensor of [n𝑛n, B/n𝐵𝑛B/n, dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}].

einsum​([n,B/n,dm​o​d​e​l],[n,B/n,E,C],dimension=[B/n])einsum𝑛𝐵𝑛subscript𝑑𝑚𝑜𝑑𝑒𝑙𝑛𝐵𝑛𝐸𝐶dimensiondelimited-[]𝐵𝑛\text{einsum}([n,B/n,d_{model}],[n,B/n,E,C],\text{dimension}=[B/n])

(7)

resulting in the final tensor of shape [n𝑛n, E𝐸E, C𝐶C, dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}], which is sharded across the first dimension.
Because each core has its own expert, we do an all-to-all communication of size [E𝐸E, C𝐶C, dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}] to now shard the E𝐸E dimension instead of the n𝑛n-dimension.
There are additional communication costs of bfloat16 tensors of size E×C×dm​o​d​e​l𝐸𝐶subscript𝑑𝑚𝑜𝑑𝑒𝑙E\times C\times d_{model} in the forward pass to analogusly receive the tokens from each expert located on different cores.
See Appendix F for a detailed analysis of the expert partitioning code.
...
In order to increase FLOPs, we must also increase the df​fsubscript𝑑𝑓𝑓d_{ff} dimension (which also increases parameters, but at a slower rate).
This presents a trade-off: as we increase df​fsubscript𝑑𝑓𝑓d_{ff} we will run out of memory per core, which then necessitates increasing m𝑚m.
...
The parameters, FLOPs per sequence and hyper-parameters of the two different models are listed below in Table 9.
Standard hyper-parameters of the Transformer, including dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}, df​fsubscript𝑑𝑓𝑓d_{ff}, dk​vsubscript𝑑𝑘𝑣d_{kv}, number of heads and number of layers are described, as well as a less common feature, F​F​NG​E​G​L​U𝐹𝐹subscript𝑁𝐺𝐸𝐺𝐿𝑈FFN_{GEGLU}, which refers to a variation of the FFN layer where the expansion matrix is substituted with two sets of weights which are non-linearly combined (Shazeer, 2020).

Model
Parameters
FLOPs/seq
dm​o​d​e​lsubscript𝑑𝑚𝑜𝑑𝑒𝑙d_{model}
F​F​NG​E​G​L​U𝐹𝐹subscript𝑁𝐺𝐸𝐺𝐿𝑈FFN_{GEGLU}
df​fsubscript𝑑𝑓𝑓d_{ff}
dk​vsubscript𝑑𝑘𝑣d_{kv}
Num. Heads
...
The Switch-C model is designed using only expert-parallelism, and no model-parallelism, as described earlier in Section 5.4.
...
We study this issue more in Appendix E.
...
Alternatively, Harlap et al. (2018); Huang et al. (2019) propose using pipeline based model parallelism, where different layers are split across devices and micro-batches are pipelined to the different layers.
...
Sparsity along the sequence length dimension (L𝐿L) in the Transformer attention patterns has been a successful technique to reduce the attention complexity from O​(L2)𝑂superscript𝐿2O(L^{2}) (Child et al., 2019; Correia et al., 2019; Sukhbaatar et al., 2019; Kitaev et al., 2020; Zaheer et al., 2020; Beltagy et al., 2020). This has enabled learning longer sequences than previously possible. This version of the Switch Transformer does not employ attention sparsity, but these techniques are complimentary, and, as future work, these could be combined to potentially improve learning on tasks requiring long contexts.
...
Though this work has focused on extremely large models, we also find that models with as few as two experts improves performance while easily fitting within memory constraints of commonly available GPUs or TPUs (details in Appendix D).
...
On a time basis, Switch Transformers can be far more efficient than dense-models with sharded parameters (Figure 6).
...
In Appendix A, we report quality improvement adding these inside Self-Attention layers, where our layer replaces the weight matrices which produce Q, K, V.
...
Shazeer et al. (2018); Lepikhin et al. (2020) designed MoE Transformers (Shazeer et al., 2017) by adding MoE layers into the dense feedfoward network (FFN) computations of the Transformer.
...
To do so, we replace the trainable weight matrices that produce the queries, keys and values with Switch layers as seen in Figure 10.

Table 10 records the quality after a fixed number of steps as well as training time for several variants.
...
Lepikhin et al. (2020) adapts a Mixture-of-Expert model and addresses expert overflow by passing its representation to the next layer without processing through a residual connection which we also follow.
...
As in reinforcement learning, a classic exploration-exploitation dilemma arises (Sutton and Barto, 2018).
These issues have been similarly noted and addressed differently by Rosenbaum et al. (2017) which demonstrated success in multi-task learning.
This particular setting most closely matches that of a contextual bandit (Robbins, 1952).
...
The resulting impact on model quality is reported in Table 11.
...
Figure 13 presents the correlation of the upstream model quality, for both dense and Switch models, on the C4 pre-training task with two downstream task measures: average SuperGLUE performance and TriviaQA score.
...
However, in the largest model regime (T5-11B/T5-XXL) our largest Switch models, as mentioned in Section 5.6, do not always translate their upstream perplexity well to downstream fine-tuning on the SuperGLUE task.
...
Generated on Sat Mar 2 07:44:35 2024 by LaTeXML
