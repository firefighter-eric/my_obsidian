# DeepSeek

## 简介

DeepSeek 是当前知识库中连接 `DeepSeek-V3` 与 `DeepSeek-R1` 的模型家族概念页。它同时覆盖高效预训练骨干与面向推理能力的强化学习路线。

## 关键属性

- 类型：模型家族 / 研究系列
- 代表来源：
  - [Unknown - 2024 - DeepSeek-V3 Technical Report](../../raw/summary/Unknown%20-%202024%20-%20DeepSeek-V3%20Technical%20Report.md)
  - [Unknown - 2024 - DeepSeek-R1 Incentivizing Reasoning Capability in LLMs via Reinforcement Learning](../../raw/summary/Unknown%20-%202024%20-%20DeepSeek-R1%20Incentivizing%20Reasoning%20Capability%20in%20LLMs%20via%20Reinforcement%20Learning.md)
- 当前角色：连接 `LLM预训练` 与 `LLM RL` 的代表性系列

## 相关主张

- `DeepSeek-V3` 体现的是 MoE、训练效率和强开源模型能力。
- `DeepSeek-R1` 体现的是“用 RL 直接激励推理能力”，并进一步延伸到蒸馏模型。
- 因此在当前知识库里，DeepSeek 不是单一模型名，而是“预训练骨干 + 推理强化学习”的系列入口。

## 来源支持

- [Unknown - 2024 - DeepSeek-V3 Technical Report](../../raw/summary/Unknown%20-%202024%20-%20DeepSeek-V3%20Technical%20Report.md)
- [Unknown - 2024 - DeepSeek-R1 Incentivizing Reasoning Capability in LLMs via Reinforcement Learning](../../raw/summary/Unknown%20-%202024%20-%20DeepSeek-R1%20Incentivizing%20Reasoning%20Capability%20in%20LLMs%20via%20Reinforcement%20Learning.md)
- [LLM 预训练](LLM%20预训练.md)
- [LLM RL](../topics/LLM%20RL.md)

## 关联页面

- [DeepSeek-V3](./DeepSeek-V3.md)
- [DeepSeek-R1](./DeepSeek-R1.md)
- [LLM 预训练](LLM%20预训练.md)
- [LLM RL](../topics/LLM%20RL.md)
- [LLM 基础脉络](../topics/LLM%20基础脉络.md)
