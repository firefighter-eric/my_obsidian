...
- 图像生成支线：`Qwen-Image` 说明 Qwen 的多模态战略并不只限于“理解型多模态”。这条线先以 `20B MMDiT`、复杂文本渲染、双语排版和精确编辑切入生产设计与视觉创作基础模型；到 `Qwen-Image-Layered`，又进一步把图像分解成多层 `RGBA` 表示，说明这条支线已经从 RGB 生成走向可编辑图层工作流。
...
- `Qwen-Image-Layered` 已把图层分解与 `RGBA` 工作流纳入 Qwen 图像分支，但当前仍缺与 `Qwen-Image` 技术报告级别的统一论述，二者在家族内部的长期关系仍需更多来源澄清。
